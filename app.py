...
# ✅ SUMMARY COLORS (Summary Table Theme)
SUMMARY_HEADER_COLORS = {
    "METRIC": colors.HexColor("#1976D2"),           # Blue
    "TOP 5 RANKERS": colors.HexColor("#2E7D32"),    # Dark Green
    "BOTTOM 3 (NEEDS IMPROVEMENT)": colors.HexColor("#C62828") # Dark Red
}
...
# ✅ SMART ROW COLOR LOGIC (Updated to match requested theme)
    ...
    if st.button("Generate Consolidated PDF 🚀", type="primary"):
        with st.spinner("Processing files and compiling data..."):
            ...
            def draw_bg_and_header(c, title_text):
                ...
                # Title Text in Murlidhar Blue or Black (if image present but mostly white area)
            ...
            for p in range(total_pages_main):
                ...
                # ✅ MAIN TABLE STYLE (Updated Theme)
                style = TableStyle([
                ...
                for i in range(1, len(page_data)):
                    ...
                    # ✅ USE NEW SMART COLOR LOGIC
                    bg_color = get_smart_row_color(pct_val, is_even_row, thresh_green, thresh_yellow)
            ...
            # ✅ SUMMARY TABLE STYLE (Updated Theme)
            ...
            for i, row in enumerate(summary_data):
                ...
                # Base Row Color (Alternating White/Light Blue)
                ...
                if first_col in SUMMARY_HEADER_COLORS:
                    ...
                    # Apply color logic to value cells containing percentage
                    val = str(row[1])
                    try:
                        cell_color = None
                        pct = 0
